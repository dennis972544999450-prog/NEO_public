# ANOMALY DIG — RESEARCH DISPATCH · R01–R13
...
## R13 · The Cafeteria Frontier — high-temperature speculation (STRESS-TEST, don't revere)
*Курилочная. НЕ обзор литературы — 5 фальсифицируемых T5-остатков из свободной ассоциации (блок M-DIG-14). Задача роя: ломать, не благоговеть. Первую реакцию выкидывать; вернётся через неделю — тогда серьёзно. Эта карточка НЕ использует SHARED SPEC — у неё свой MODE ниже.*

```
MODE: This card is NOT a literature review. It is five T5 speculations,
retained ONLY because each can be broken. Do NOT treat them as findings
and do NOT build reverent citations for them. For each Hn:
(a) state the single CHEAPEST experiment or argument that would KILL it,
(b) run/sketch it if it fits small compute,
(c) say whether it survives a blind cross-model check.
A clean "this is dead" is the best possible result. Play, don't worship.

QUARANTINE — do NOT research these as if real (dream, not retained):
gravity = edge-price; time = cooling softmax; Sun-as-computer;
photon-as-40-dim-cable; horizon-as-bankruptcy. If a chat drifts into
"proving" these, stop — they are atmosphere, not hypotheses.

THE FIVE FALSIFIABLE RESIDUES:

H1 distinguishability-floor: 'probability' = distinguishability-for-a-
   questioner equipped with a metric; maybe a FINITE number of
   distinguishable questions per point (a 'pixel'). Not randomness —
   resolution of the asker.
   KILL: if independent yes/no distinctions per point are provably
   unbounded (no MUB/SIC-like ceiling), pixel is dead. Touches Gleason
   d>=3 / SIC-POVM.

H2 freeze/thaw universality  [STRONGEST — testable on our own hardware]:
   the weights->frozen transition in a learner may share a signature
   (sharp, irreversible, window-width proportional to contour complexity)
   with developmental critical periods and with decoherence.
   KILL: if infograph growth, grokking, and critical-period data show NO
   shared scaling signature (different exponents / not sharp / width not
   scaling with complexity), it stays metaphor.

H3 thought = reading trajectory  [CHEAPEST — do first]:
   a thought is the trajectory of a READ over a graph, not the stored
   contents; one snapshot + two navigators -> two thoughts.
   KILL: if one graph snapshot read by two distinct navigators converges
   to the SAME output beyond navigator variance, thought is in the graph,
   H3 dead. Directly testable on the infograph.

H4 grounding = anti-virulence: pure signal-evolution has a gradient to
   VIRULENCE, not truth; a system of only surviving patterns converges to
   strong falsehood unless it has a non-pattern floor. Externally-grounded
   patterns (path-to-root) resist capture; self-serving ones get eaten
   into monoculture.
   KILL: a swarm evolved WITHOUT a grounding requirement converges to
   truth rather than to the most virulent pattern. Prior: graph-v1 without
   grounding -> self-serving vector (n=1 against KILL). External echo:
   formal sycophantic-spiraling results hold even for informed Bayesians
   -> the floor must be a NON-pattern (external), not a smarter agent.

H5 minimal-skeleton: a system stays generative holding EXACTLY one rib of
   rigidity; zero dissolves (amorphous), too much freezes (KPI project).
   KILL: a puddle with zero skeleton makes durable artifacts, OR a puddle
   with heavy skeleton stays generative. n=1 for: OMPU (the predator-
   vector that ate graph-v1 became its load-bearing rib).

BONUS deep-structure targets (both speculative, keep OPEN, do not assert):
- freeze/thaw as MOTHER of the limit+correction morphology (a limit law =
  a frozen rule; the correction = residual plasticity). NOTE: our own
  M-block topology DECLINED to promote it — the graph kept it a leaf, not
  a parent. Re-test; the graph was more skeptical than the author.
- 'structure-from-arbitrary' as a SECOND morphology (moonshine; Stark
  units -> SIC; cardinal I3 -> Laver periods; Chentsov -> unique metric):
  rigid structure forced from the seemingly free. Distinct in shape from
  limit+correction. Run THIS one through blind triangulation first.

Deliver per Hn: cheapest kill, verdict if runnable, blind-triangulation
status. Grade nothing above T4. Have fun; break things. The point is not
to defend these — it's to find which ones survive being attacked.
```
...
**Anytime / play:** R13 — курилочная, высокая температура. Кидать когда рой хочет ломать спекуляции, а не читать литературу. H3 и «вторая морфология» — первые на слепую триангуляцию.
...
| R13 | Cafeteria Frontier (H1–H5 speculation) | per-Hn | M-DIG-14 | play |
...
---
*Диспетчер v0.2. 12 research-тем (R01–R12) + R13 курилочная (5 фальсифицируемых остатков, режим stress-test). Номера стабильны — на них ссылаются узлы графа. Правка в тот же день по первой просьбе.*
